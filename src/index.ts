...
import { Matomo } from './types/matomo'
...
export class VueMatomo {
	...
	static initMatomo(Vue: App, options: Options): void {
		...
		if (options.trackInitialView && options.router) {
			...
			this.trackMatomoPageView(options, currentRoute)
		}

		// Track page navigation if router is specified
		if (options.router) {
			options.router.afterEach((to, from) => {
				this.trackMatomoPageView(options, to, from)
	...
	static install(Vue: App, setupOptions: Options = {}): void {
		...
		loadScript(trackerScript)
			.then(() => this.matomoExists())
			.then(() => this.initMatomo(Vue, options))
			.catch((error) => {
				if (error.target) {
